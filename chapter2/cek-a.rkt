...
(provide interp)
(define (interp exp1)
  (value-of/k (parse (macro-translate exp1))
              (empty-env)
              (end-cont)))
...
(define (parse exp1)
  ...
  (match exp1
    ...
    [`(lambda ,args ,body)
     (lambda (env cont)
       (apply-cont cont (closure args (>> body) env)))]
    ; an if expression
    [`(if ,e1 ,e2 ,e3)
     (lambda (env cont)
    ...
    [`(fix ,f ,args ,body)
     (lambda (env cont)
       (let ((cbody (>> body)))
         (apply-cont cont
                     (closure args cbody (extendrec-env env f args cbody)))))]
    ; an application
    [`(,ef . ,exps)
     (lambda (env cont)
       (value-of/k (>> ef) env (args-cont cont env (map >> exps))))]))

(define (value-of/k exp1 env cont) (exp1 env cont))
...
; opt
(struct opt-proc (name opt) #:transparent)
...
(define opt-table
  (map (lambda (p)
         (cons (car p) (opt-proc (car p) (cdr p))))
       (list
        (cons '+ +)
        (cons '- -)
...
(define (apply-opt/k opt vals cont)
  (apply-cont cont (apply (opt-proc-opt opt) vals)))
